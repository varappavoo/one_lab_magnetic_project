DefineConstant[
  NumMagnets = {57, Min 1, Max 60, Step 1, Name "Parameters/0Number of magnets"}
];
mm = 1.e-3;
...
For i In {1:NumMagnets}
	...
	shift_y = 50*mm;
	shift_z = 50*mm;
	If(count > 18 && count < 38)
		Printf(">18 <38: count= %f",count);
		DefineConstant[
	...
	If(count > 37)
		Printf(">37: count= %f",count);
		DefineConstant[
			X~{i} = {shift_x, Min -100*mm, Max 100*mm, Step mm,
			  Name Sprintf("Parameters/Magnet %g/0X position [m]", i) },
			Y~{i} = {x(count-38)*45*mm + shift_y, Min -100*mm, Max 100*mm, Step mm,
			  Name Sprintf("Parameters/Magnet %g/0Y position [m]", i) },
			Z~{i} = {z(count-38)*45*mm, Min -100*mm, Max 100*mm, Step mm,
			  Name Sprintf("Parameters/Magnet %g/0Z position [m]", i) },
