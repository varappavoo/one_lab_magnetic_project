  ...
  { Name MagSta_phi ; NameOfPostProcessing MagSta_phi;
    Operation {
      ...
      */
      ...
      // MY LINE IS AN ARC (SEMICIRCLE)
      // X,Y,Z,MESH
      Point(1) = {0, 0, 0, 1}; // centre of circle
      Point(2) = {-0.1, 0, 0, 1};
      Point(3) = {0, 0.1, 0, 1};
      Point(4) = {0.1, 0, 0, 1};

      // CREATING SEMI CIRCLES BY CONNECTING ARCS
      // FIRST, CENTRE, LAST
      Circle(1) = {2, 1, 3};
      Circle(2) = {3, 1, 4};
      Line Loop(1) = {1, 2};
      Print[ b,   OnLine {Loop(1)}{100}, File >"b_mxp_circle_centre.dat.csv", Format Table ] ;
        
      /*
      For i In {-10:10}
        //Print[ b,   OnLine {{-0.10, 0.01 * i, 0.10}{0.10, 0.01 * i, -0.10}}{30}, File >"b_mxp_diag.dat.csv", Format Table ] ;
        //Print[ b,   OnLine {{-0.10, 0.01 * i, 0}{0.10, -0.01 * i, 0}}{30}, File >"b_mxp_hori_x.dat.csv", Format Table ] ;
